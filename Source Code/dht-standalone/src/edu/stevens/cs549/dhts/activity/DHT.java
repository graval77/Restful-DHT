...
//		routing.joinCheck();
	...
	//to find the closest successor from the Routing table
...
//				System.out.println(id + "  :  " + info + "  :  " + succ);
				succ = getSucc(info);
				/*
	...
	public TableRep notify(TableRep predDb) {
		...
		NodeInfo info = getNodeInfo();
		boolean predFailed = false;
		//To change the predFailed value to true if the predecessor pointer is not pointing to "null"
		if(pred != null) {
			predFailed = this.client.isFailed(pred.addr);
		}
		if (pred == null || inInterval(cand.id, pred.id, info.id, false) || predFailed) {
			setPred(cand);
			/*
...
//		}
	...
	//to find the Successor to the Node with the help of the id from the NodeInfo
    ...
    //to check the Successor to the Node from the NodeInfo class 
	public void checkSuccessor() {
		
		NodeInfo succ = getSucc();
		boolean failed = this.client.isFailed(succ.addr);
		if(failed) {
			info("Fixing the dropped node " + succ);
