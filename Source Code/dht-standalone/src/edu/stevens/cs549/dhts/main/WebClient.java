...
public class WebClient {
	...
	//to get the Successor of the node 
	...
	public NodeInfo findSuccessor(URI addr, int id) throws Failed {
		URI sucessorURI = UriBuilder.fromUri(addr).path("find").queryParam("id", id).build();
		info("client getPred(" + sucessorURI + ")");
		Response response = getRequest(sucessorURI);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET /pred");
		} else {
	...
	//to find the closest preceding Finger value from the NodeInfo
	public NodeInfo findClosestPrecedeFinger(URI addr, int id) throws Failed {
		URI sucessorURI = UriBuilder.fromUri(addr).path("closestfinger").queryParam("id", id).build();
		info("client findClosestPrecedeFinger(" + sucessorURI + ")");
		Response response = getRequest(sucessorURI);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET /pred");
		} else {
	 ...
	 */
	//to get the bindings in the network
	public String[] getKey(URI addr, String key) throws Failed {
		URI sucessorURI = UriBuilder.fromUri(addr).queryParam("key", key).build();
		info("client getKey(" + sucessorURI + ")");
		Response response = getRequest(sucessorURI);
		if (response == null || response.getStatus() >= 300) {
	...
	//to put the bindings in the network
	public void putKey(URI addr, String key, String value) throws Failed {
		URI sucessorURI = UriBuilder.fromUri(addr).queryParam("key", key).queryParam("value", value).build();
		info("client putKey(" + sucessorURI + ")");
		Response response = putRequest(sucessorURI);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET /putKey");
		} 
	}
    //to delete the bindings from the network
	public void deleteKey(URI addr, String key, String value) throws Failed {
		URI sucessorURI = UriBuilder.fromUri(addr).queryParam("key", key).queryParam("value", value).build();
		info("client deleteKey(" + sucessorURI + ")");
		Response response = deleteRequest(sucessorURI);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET /putKey");
		} 
	}
